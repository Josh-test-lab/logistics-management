...
```{r}
# 平假日路線各上車站點票種直方疊圖 (去程，driction = 0; 回程，driction = 1)
tick_type_week_day_driection_histgram <- function(bus, year, week_day, driection) {
  months = generate_months(year, year, month.from, month.to)
  if (week_day == '平日'){
  ...
  df_ticket <- df_ticket[df_ticket$Weekday %in% weekday, ]
  ticket_count <- convert_holder_type(df_ticket$HolderType) %>% table()
  color <- gray.colors(length(ticket_count))
  ...
  barplot(height = ticket_count,
                ...
                yaxt = "n",
                main = "",
                ylim = c(0, max(ticket_count) * 1.1)
                )

  title(main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線', ifelse(driection == 0, '去程', '回程'), ifelse(week_day == '平日', '平日', '假日'), '各上車站點票種直方疊圖'), cex.main = 2, adj = 0)
...
```{r}
# 上車分鐘票種點圖
boarding_time_driection_plot <- function(bus, year, driection) {
  months = generate_months(year, year, month.from, month.to)
  ...
  title(main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線(', ifelse(driection == 0, '去程', '回程'), ')上車站點-上車分鐘票種點圖'), cex.main = 2, adj = 0)
  
  # x 軸
  labels_to_use <- if (driection == 0) {
    bus_stops[[bus]]
  } else {
    rev(bus_stops[[bus]])
  }
...
for (driection in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      # 繪圖 (在 R中呈現)
      boarding_time_driection_plot(bus, year, driection)
      
      # 儲存圖片
      path = paste0("images/", year - 1911, "/", bus, "/scatter plot")
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(driection == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_all.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_driection_plot(bus, year, driection)
      dev.off()
    }
  }
...
for (driection in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      ...
      path = paste0("images/", year - 1911, "/", bus, "/scatter plot")
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(driection == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_up.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_driection_plot(bus, year, driection)
      dev.off()
    }
  }
...
for (driection in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      ...
      path = paste0("images/", year - 1911, "/", bus, "/scatter plot")
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(driection == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_down.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_driection_plot(bus, year, driection)
      dev.off()
    }
  }
