...
volume_by_bus_plot <- function(bus, year.from, year.to) {
  ...
  title(main = paste0(year.from - 1911, '年', '至', year.to - 1911, '年', bus, '路線總運量'), cex.main = 2, adj = 0)
...
```{r}
# 平假日路線各上車站點票種直方疊圖 (去程，direction = 0; 回程，direction = 1)
tick_type_week_day_direction_histgram <- function(bus, year, week_day, direction) {
  months = generate_months(year, year, month.from, month.to)
  if (week_day == '平日'){
  ...
  par(family = "kai", mar = c(5, 13, 4, 6))  # 字體與圖寬
  ...
  title(main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線', ifelse(direction == 0, '去程', '回程'), ifelse(week_day == '平日', '平日', '假日'), '各上車站點票種直方疊圖'), cex.main = 2, adj = 0)
...
for (week_day in c('平日', '假日')){
  for (direction in 0:1){
    for(bus in buses){
      for (year in year.from:year.to){
        # 繪圖 (在 R中呈現)
        tick_type_week_day_direction_histgram(bus, year, week_day, direction)
        
        # 儲存圖片
        path = paste0("images/", year - 1911, "/", bus, "/barplot")
        check_path(path)
        png(filename = paste0(path, "/路線", ifelse(direction == 0, '去程', '回程'), ifelse(week_day == '平日', '平日', '假日'),  "各上車站點票種直方疊圖_all.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
        tick_type_week_day_direction_histgram(bus, year, week_day, direction)
        dev.off()
      }
    }
  ...
  for (direction in 0:1){
    for(bus in buses){
      for (year in year.from:year.to){
        ...
        path = paste0("images/", year - 1911, "/", bus, "/barplot")
        check_path(path)
        png(filename = paste0(path, "/路線", ifelse(direction == 0, '去程', '回程'), ifelse(week_day == '平日', '平日', '假日'),  "各上車站點票種直方疊圖_up.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
        tick_type_week_day_direction_histgram(bus, year, week_day, direction)
        dev.off()
      }
    }
  ...
  for (direction in 0:1){
    for(bus in buses){
      for (year in year.from:year.to){
        ...
        path = paste0("images/", year - 1911, "/", bus, "/barplot")
        check_path(path)
        png(filename = paste0(path, "/路線", ifelse(direction == 0, '去程', '回程'), ifelse(week_day == '平日', '平日', '假日'),  "各上車站點票種直方疊圖_down.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
        tick_type_week_day_direction_histgram(bus, year, week_day, direction)
        dev.off()
      }
    }
...
boarding_time_direction_plot <- function(bus, year, direction) {
  ...
  df_ticket$Color <- color[df_ticket$HolderType]
  
  par(family = "kai", mar = c(14, 6, 4, 10))  # 字體與圖寬
  
  plot(x = df_ticket$BoardingStopSequence,
       y = df_ticket$BoardingTimeInMinutes,
       col = df_ticket$Color,
       xlab = "",
       ylab = "上車分鐘",
       pch = 16,
  ...
  title(main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線(', ifelse(direction == 0, '去程', '回程'), ')上車站點-上車分鐘票種點圖'), cex.main = 2, adj = 0)
...
for (direction in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      ...
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(direction == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_all.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_direction_plot(bus, year, direction)
      dev.off()
    }
  }
...
for (direction in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      ...
      path = paste0("images/", year - 1911, "/", bus, "/scatter plot")
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(direction == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_up.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_direction_plot(bus, year, direction)
      dev.off()
    }
  }
...
for (direction in 0:1){
  for(bus in buses){
    for (year in year.from:year.to){
      ...
      path = paste0("images/", year - 1911, "/", bus, "/scatter plot")
      check_path(path)
      png(filename = paste0(path, "/路線(", ifelse(direction == 0, '去程', '回程'), ")上車站點-上車分鐘票種點圖_down.png"), width = 13.79, height = 8.25, units = "in", res = 300, family = "kai")
      boarding_time_direction_plot(bus, year, direction)
      dev.off()
    }
  }
