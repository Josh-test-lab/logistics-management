...
year_total_volume_plot <- function(buses, year.from, year.to) {
  ...
  plot(x = years,
       ...
       xlab = "年",
       ylab = "搭乘次數",
       ylim = c(min(sapply(transport_volume_year, min)) * 0.9 ,
                max(sapply(transport_volume_year, max)) * 1.1),
       cex.main = 2,
       cex.lab = 2,
       cex.axis = 1.5,
       cex = 1.5,
...
for(bus in buses){
  for(month_index in months){
    tpass = rbind(tpass, df[[month_index]][[bus]])
  }
}

tpass = tpass[!is.na(tpass$SubTicketType), ]
tpass$BoardingTime = ymd_hms(tpass$BoardingTime)
