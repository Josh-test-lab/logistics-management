...
```{r}
# 運量
volume_plot <- function(bus, year, month.from, month.to) {
...
generate_volume_plots <- function(buses, year.from, year.to, month.from, month.to) {
  for (year in year.from:year.to) {
    for (bus in buses) {
      ...
      # 繪圖 (在 R 中呈現)
      volume_plot(bus, year, month.from, month.to)
      
      # 儲存圖片
      path <- paste0("images/", year - 1911, "/", bus, "/line chart")
      check_path(path)
      img_filename <- paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線搭乘次數折線圖')
...
```{r}
# TPASS 運量
tpass_volume_plot <- function(bus, year, month.from, month.to) {
  months = generate_months(year, year, month.from, month.to)
  transport_volume = NULL
  tpass = NULL
...
generate_tpass_volume_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for (year in year.from:year.to){
    ...
    # 繪圖 (在 R中呈現)
      tpass_volume_plot(bus, year, month.from, month.to)
      
    # 儲存圖片
      path = paste0("images/", year - 1911, "/", bus, "/line chart")
      check_path(path)
      img_filename = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線TPASS搭乘次數折線圖')
      png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
      tpass_volume_plot(bus, year, month.from, month.to)
      dev.off()
    }
...
```{r}
# TPASS 日均運量
average_daily_tpass_volume_plot <- function(bus, year, month.from, month.to) {
  months = generate_months(year, year, month.from, month.to)
  transport_volume = NULL
  tpass = NULL
  month.days = NULL
...
generate_average_daily_tpass_volume_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for (year in year.from:year.to){
    ...
    # 繪圖 (在 R中呈現)
      average_daily_tpass_volume_plot(bus, year, month.from, month.to)
    
    # 儲存圖片
      path = paste0("images/", year - 1911, "/", bus, "/line chart")
      check_path(path)
      img_filename = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線TPASS日均搭乘次數折線圖')
      png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
      average_daily_tpass_volume_plot(bus, year, month.from, month.to)
      dev.off()
    }
...
```{r}
# 平假日搭乘次數
weekend_and_weekday_volume_plot <- function(bus, year, month.from, month.to) {
  months = generate_months(year, year, month.from, month.to)
  weekend.days = NULL
  weekday.days = NULL
  month.days = NULL
...
generate_weekend_and_weekday_volume_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for (year in year.from:year.to){
    ...
    # 繪圖 (在 R中呈現)
      weekend_and_weekday_volume_plot(bus, year, month.from, month.to)
    
    # 儲存圖片
      path = paste0("images/", year - 1911, "/", bus, "/line chart")
      check_path(path)
      img_filename = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線日均搭乘次數折線圖')
      png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
      weekend_and_weekday_volume_plot(bus, year, month.from, month.to)
      dev.off()
    }
...
```{r}
# 太魯閣客運單一年總運量
total_volume_plot <- function(buses, year, month.from, month.to) {
  months = generate_months(year, year, month.from, month.to)
...
generate_total_volume_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for (year in year.from:year.to){
    # 繪圖 (在 R中呈現)
    total_volume_plot(buses, year, month.from, month.to)
    
    # 儲存圖片
    path = paste0("images/", year - 1911, "/total/line chart")
    check_path(path)
    img_filename = paste0(year - 1911, '年', month.from, '月至', month.to, '月太魯閣客運總運量')
    png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
    total_volume_plot(buses, year, month.from, month.to)
    dev.off()
  }
...
generate_volume_by_bus_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for(bus in buses){
    ...
    check_path(path)
    img_filename = paste0(year.from - 1911, '年', '至', year.to - 1911, '年', month.from, '月至', month.to, '月', bus, '路線月運量')
    png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
    volume_by_bus_plot(bus, year.from, year.to, month.from, month.to)
    dev.off()
  }
...
```{r}
# 月增長量
monthly_gorw_up_plot <- function(bus, year, month.from, month.to) {
  color <- "gray"
  ...
  plot(x = month.from:month.to,
       ...
       xlab = "月份",
       ylab = "成長人數",
       ylim = c(min(df_grow_year) * ifelse(min(df_grow_year) > 0, 0.9, 1.1) , max(df_grow_year) * 1.2),
       cex.main = 2,
       cex.lab = 2,
       cex.axis = 1.5,
       cex = 1.5,
  ...
  title(main = paste0(year - 1 - 1911, '、', year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線運量消長情形同期比較'), cex.main = 2, adj = 0)
...
generate_monthly_gorw_up_plot <- function(buses, year.from, year.to, month.from, month.to) {
  for(bus in buses){
    for (year in (year.from + 1):year.to){
      # 繪圖 (在 R中呈現)
      monthly_gorw_up_plot(bus, year, month.from, month.to)
      
      # 儲存圖片
      path = paste0("images/", year - 1911, "/", bus, "/line chart")
      check_path(path)
      img_filename = paste0(year - 1 - 1911, '、', year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線運量消長情形同期比較')
      png(filename = paste0(path, "/", img_filename, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
      monthly_gorw_up_plot(bus, year, month.from, month.to)
      dev.off()
    }
...
```{r}
# 月平假日增長量
weekday_and_weekend_gorw_up_plot <- function(bus, year, month.from, month.to) {
  color <- gray.colors(3)
  ...
  title(main = paste0(year - 1 - 1911, '、', year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線平假日日均次數消長情形'), cex.main = 2, adj = 0)
