...
for (year in year.from:year.to){
  ...
  # 儲存圖片
    path = paste0("images/", year - 1911, "/", bus, "/line_chart")
    check_path(path)
    main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線月運量及TPASS搭乘次數折線圖')
    png(filename = paste0(path, "/", main, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
    tpass_volume_plot(bus, year)
    dev.off()
  }
...
for (year in year.from:year.to){
  ...
  # 儲存圖片
    path = paste0("images/", year - 1911, "/", bus, "/line_chart")
    check_path(path)
    main = paste0(year - 1911, '年', month.from, '月至', month.to, '月', bus, '路線日均運量及TPASS搭乘次數折線圖')
    png(filename = paste0(path, "/", main, ".png"), width = 15, height = 5, units = "in", res = 300, family = "kai")
    tpass_volume_plot(bus, year)
    dev.off()
  }
...
combined_df$上車時間 = ymd_hms(combined_df$上車時間)

mytable <- table(year(combined_df$上車時間) - 1911, combined_df$路線編號)
